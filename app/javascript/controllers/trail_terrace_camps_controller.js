...
export default class extends Controller {
  ...
  static values = { page: { type: Number, default: 0 }, pageSize: { type: Number, default: 2 } }
  ...
  next(event) {
    event.preventDefault()
    if (this.pageValue >= this.pageCount - 1) return
    this.pageValue += 1
    this.renderPage()
  }
  ...
  syncChrome() {
    ...
    if (this.hasDotsTarget) this.dotsTarget.hidden = !multiPage

    if (!multiPage) return

    if (this.hasPrevTarget) this.prevTarget.hidden = page <= 0
    if (this.hasNextTarget) this.nextTarget.hidden = page >= this.pageCount - 1
