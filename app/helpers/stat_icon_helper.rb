module StatIconHelper
  ...
  def stat_icon_for(habit)
    ...
    return "🚶" if unit.match?(/step/) || name.match?(/walk|step|run|jog/)
    return "📚" if unit.match?(/page|chapter|book/) || name.match?(/read|page|book/)
    ...
    return "📝" if name.match?(/journal|writ|note|diary/)
  ...
  def amount_delta_label(habit)
    ...
    if habit.healthy_range?
      return "In range" if habit.status == :perfect
      return "Below range" if habit.status == :too_low
      return "Above range" if habit.status == :too_high
    end

    return "Same as yesterday" if delta.zero?

    sign = delta.positive? ? "+" : ""
    "#{sign}#{format_amount(delta)} #{unit} vs yesterday"
  end
end
