...
module MountainTrailHelper
  ...
  BUILTIN_TENT_SLOTS = [
    ...
    { x: 0.500, y: 0.710, width: 0.15 },
    { x: 0.410, y: 0.550, width: 0.11 }
  ].freeze
  ...
  def mountain_trail_reveal_camps(trail_or_projects, builtin_map: false)
    nodes =
      if trail_or_projects.respond_to?(:visible_nodes)
        ...
        # Legacy: treat as project list and invent window nodes by position.
        Array(trail_or_projects).map.with_index do |project, index|
  ...
  def mountain_trail_dock_card(projects: [], open_battles: [], won_today: 0, journey: nil, user: nil)
    ...
    if waiting.any? && stage_camps.empty?
      first = waiting.first
      return meadow_plaque(
        mode: "win_next",
        headline: first.try(:title).to_s.presence ||
