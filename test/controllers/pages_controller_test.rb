...
class PagesControllerTest < ActionDispatch::IntegrationTest
  test "landing page is public" do
    ...
    assert_match(/Planning counts too/, response.body)
    assert_match(/role="img"/, response.body)
    assert_no_match(/How it works/, response.body)
    assert_no_match(/Run my first 10 km/, response.body)
    assert_no_match(/Name today(?:&#39;|')s battle/i, response.body)
    assert_no_match(/Win the battle\. Your mountain rises/, response.body)
    assert_no_match(/landing-scene-climb/, response.body)
