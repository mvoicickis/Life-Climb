...
class StrategyGoalsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "journey show includes strategy board and universe strip" do
    get life_journey_path(@journey)
    assert_response :success
    assert_match(/Strategy/i, response.body)
    assert_match(/Break the goal down/i, response.body)
    assert_select ".lp-strategy__universe"
    assert_select ".lp-strategy__horizon", minimum: 4
  end
  ...
  test "year month week day cascade awards SP and fills today feeder" do
    ...
    assert_redirected_to life_journey_path(@journey, area: "career")
